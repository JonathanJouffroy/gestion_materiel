...
<html>
    <head>    
        ...
        <script>
            ...
            function actualiserTableau(idAntenne) {
                $.post (
                      "../Controlleurs/c_trierStockFormationACommander.php",
                      {
                        idAntenne:idAntenne
                      }, function(data) {
                        ...
                        tabMateriel.forEach((materiel) => {
                            $("#tableMateriel").find("tbody").append('<tr data-idMateriel="'+materiel['id']+'"><td>'+materiel['nom']+'</td><td>'+materiel['nom_rnmsc']+'</td><td>'+tinyIntToBoolean(materiel['Consommable'])+'</td><td>'+materiel['quantite']+'</td><td>'+tinyIntToBoolean(materiel['packaging'])+'</td><td>'+materiel['type_formation']+'</td><td>'+materiel['localisation']+'</td><td>'+materiel['date_de_peremption']+'</td><td>'+materiel['stock_minimum']+'</td><td>'+materiel['nomAntenne']+'</td></tr>');
                        })
                      }
                );
            }
    ...
    <body>
        <div class="col-sm-12 col-xs-12">
            <h1 class="text-center">Matériel de formation à commander</h1>
