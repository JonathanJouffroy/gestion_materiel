...
<html>
        ...
        <script>
            $(document).ready(function(){
                ...
                $(document).on('click', '.modifier', function() {
                    $.post (
                      "../Controlleurs/c_modifierMaterielFormation.php",
                      {
                        ...
                        type_formation:$(this).parent().parent().find("td").find(".type_formation").val(),
                ...
                $(document).on('click', '.supprimer', function() {
                    $.post (
                      "../Controlleurs/c_supprimerMaterielFormation.php",
                      {
                        id:$(this).parent().parent().attr("data-idMateriel")
                      }, function() {
                        var idAntenne = $("#listeAntennes").val();
            ...
            function actualiserTableau(idAntenne) {
                $.post (
                      "../Controlleurs/c_trierStockFormation.php",
                      {
                        idAntenne:idAntenne
                      }, function(data) {
                        ...
                        tabMateriel.forEach((materiel) => {
                            $("#tableMateriel").find("tbody").append('<tr data-idMateriel="'+materiel['id']+'"><td><input type="text" class="nom" value="'+materiel['nom']+'"/></td><td><input type="text" class="nom_rnmsc" value="'+materiel['nom_rnmsc']+'"/></td><td>'+tinyIntToBoolean(materiel['Consommable'])+'</td><td><input type="text" class="quantite" value="'+materiel['quantite']+'"/></td><td>'+tinyIntToBoolean(materiel['packaging'])+'</td><td><input type="text" class="type_formation" value="'+materiel['type_formation']+'"/></td><td><input type="text" class="localisation" value="'+materiel['localisation']+'"/></td><td><input type="text" class="peremption" value="'+materiel['date_de_peremption']+'"/></td><td><input type="text" class="stock_minimum" value="'+materiel['stock_minimum']+'"/></td><td>'+materiel['nomAntenne']+'</td><td><input type="button" class="btn btn-primary modifier" value="Modifier"/></td><td><input type="button" class="btn btn-danger supprimer" value="Supprimer"/></td></tr>');
                        })
                      }
                );
            }
    ...
    <body>
        <div class="col-sm-12 col-xs-12">
            <h1 class="text-center">Gérer le stock de formation</h1>
            ...
            <span>
                <table class="table" id="tableMateriel">
                    ...
                    <tbody>
                        <?php
                            foreach ($materielFormation as $materiel)
                            {
                                echo '<tr data-idMateriel="'.$materiel['id'].'">
                                        ...
                                        <td><input type="text" class="type_formation" value="'.$materiel['type_formation'].'"/></td>
